...
import { Send, Loader2, Plus, Asterisk } from "lucide-react"
...
const Composer = forwardRef(function Composer({ onSend, busy }, ref) {
  ...
  // Model selector state and options
      ...
      >
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <div className="flex items-center gap-1 shrink-0">
            {/* Model selector replaces voice button */}
            <div className="relative">
              <button
                type="button"
                onClick={() => setModelOpen((o) => !o)}
                className="inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-white px-3 py-2 text-sm font-medium hover:bg-zinc-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 dark:border-zinc-800 dark:bg-zinc-950 dark:hover:bg-zinc-800"
                title="Select model"
              >
                <span className="text-sm">
                  {typeof models.find((m) => m.name === selectedModel)?.icon === "string"
              ...
              {modelOpen && (
                <div className="absolute right-0 mt-1 w-48 rounded-lg border border-zinc-200 bg-white shadow-lg dark:border-zinc-800 dark:bg-zinc-950 z-50">
                  {models.map((m) => (
                    <button
                      key={m.name}
                      onClick={() => {
                    ...
                    >
                      {typeof m.icon === "string" ? <span className="text-sm">{m.icon}</span> : m.icon}
                      {m.name}
                    </button>
                  ))}
                </div>
